<template>
  <div>
    ...
    >
      <template v-slot:item.type="{ item }">
        <div class="flex justify-between items-center">
          <div
            class="p-2 bg-yellow-100 text-yellow-600 rounded-xl inline-block mr-2"
          >
            {{ item.raw.type }}
          </div>
        </div>
      ...
      <template v-slot:item.actions="{ item }">
        <div class="flex gap-3">
          <div class="mr-2 cursor-pointer" @click="editItem()">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="h-5 w-5"
              viewBox="0 0 24 24"
    ...
    <Popup
      type="addWord"
      v-if="
        this.$store.state.showPopup && this.$store.state.typePopup === 'addWord'
      "
    >
